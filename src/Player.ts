...
export class Player extends Container {
    ...
    public useTool(): void {
        ...
        if (used && currentTool.isConsumable) {
            // Handle consumable tools
            if (currentTool instanceof TurnipSeed) {
                currentTool.consume();
                if (currentTool.count <= 0) {
                    // Replace with null to maintain empty spot in toolbag
                    this.toolBag[this.selectedToolIndex] = null;
                    console.log("Turnip Seeds depleted!");
                }
            }
        }
    }
